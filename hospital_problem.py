...
def dict_conversion(record):
    clean={}
    fields=record.split(",")
    for f in fields:
        k,v =f.split(":")
        if k in ["id","age","hr","gulocose"]:
            clean[k]=int(v)
        else:
            clean[k]=v
            return clean
        patients=[dict_conversion(r) for r in patients_data]
...
for p in patients:
    ...
    avg_hr_doctor={doc:sum(hrs)/len(hrs) for doc ,hrs in hr_per_doctor.items()}
...
#q8
distance_doctors=len(set(p["doctors"] for p in patients))
