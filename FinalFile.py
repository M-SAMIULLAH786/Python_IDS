...
def dict_conversion_fun(record):
    clean = {}
    fields = record.split(",")
    for f in fields:
        k, v = f.split(":")
...
patients = [dict_conversion_fun(r) for r in patients_data]
...
hr_per_doctor = defaultdict(list)
for p in patients:
    hr_per_doctor[p["doctor"]].append(p["hr"])

avg_hr_per_doctor = {doc: sum(hrs)/len(hrs) for doc, hrs in hr_per_doctor.items()}

# #Q4
# high_risk_patients = [p for p in patients if p["hr"] > 100]

# #Q5
# lowest_glucose_patient = min(patients, key=lambda x: x["glucose"])

# #Q6
# highest_glucose_patient = max(patients, key=lambda x: x["glucose"])
...
# bp_alert = [p for p in patients if bp_high(p["bp"])]

# # Q8
# distinct_doctors = len(set(p["doctor"] for p in patients))

# #Q9 
# feedback_count = defaultdict(int)
# for p in patients:
#     feedback_count[p["feedback"]] += 1


# #Q10 

# avg_overall_hr = sum(p["hr"] for p in patients) / len(patients)
...
# #Q12 
# for p in patients:
#     p["glucose_improved"] = round(p["glucose"] * 0.95)

# #Q13
# doctor_summary = {}
# for doc, hrs in hr_per_doctor.items():
#     glucose_vals = [p["glucose"] for p in patients if p["doctor"] == doc]
#     doctor_summary[doc] = {
#         "patients": len(hrs),
...
# senior_patients = [p for p in patients if p["age"] >= 60]



print("Total Patients:", patients)
print("Average HR per Doctor:", avg_hr_per_doctor)
# print("Lowest Glucose:", lowest_glucose_patient)
# print("Highest Glucose:", highest_glucose_patient)
